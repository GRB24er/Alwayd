// Enterprise-grade PDF receipt generator for Aldwych European Capital.
// Produces branded, QR-verified, watermarked payment receipts matching
// the quality of the web application's official documents.
...
export type ReceiptStatus = "completed" | "processing" | "pending";
...
function generateIntegrityHash(data: ReceiptData): string {
  const payload = JSON.stringify({
    receipt: data.receiptNumber, ref: data.reference,
    amount: data.amount, currency: data.currency,
    sender: data.sender.name, beneficiary: data.beneficiary.name,
    initiated: data.initiatedAt.toISOString(),
  });
...
interface Ctx {
  pdf: PDFDocument; page: PDFPage; font: PDFFont; bold: PDFFont;
  cursorY: number; logo: { width: number; height: number; embed: PDFImage } | null;
  data: ReceiptData; integrityHash: string; generatedAt: Date;
}

function statusColor(data: ReceiptData) {
  return data.status === "completed" ? GREEN : data.status === "processing" ? AMBER : MUTED;
}
function statusBadgeText(data: ReceiptData) {
  return data.status === "completed" ? "TRANSFER COMPLETED" : data.status === "processing" ? "PROCESSING — PENDING SETTLEMENT" : "PENDING";
}

function drawWatermark(page: PDFPage, bold: PDFFont, data: ReceiptData) {
  ...
  for (let row = 0; row < 9; row++) {
    for (let col = 0; col < 3; col++) {
      ...
      const y = 30 + row * 105;
      page.drawText(tile, { x, y, size: tileSize, font: bold, color: rgb(0.93, 0.9, 0.83), rotate: degrees(-30), opacity: 0.5 });
    }
  }
  const stamp = data.status === "completed" ? "COMPLETED" : data.status === "processing" ? "PROCESSING" : "PENDING";
  const stampSize = 86;
  const w = bold.widthOfTextAtSize(stamp, stampSize);
  page.drawText(stamp, {
    x: PAGE_W / 2 - w / 2 + 60, y: PAGE_H / 2 - 20, size: stampSize, font: bold,
    color: data.status === "completed" ? rgb(0.78, 0.9, 0.84) : rgb(0.93, 0.88, 0.78), rotate: degrees(-30), opacity: 0.22,
  });
}
...
function drawHeader(ctx: Ctx) {
  ...
  if (logo) {
    const gw = 240; const gh = gw * (logo.height / logo.width);
    page.drawImage(logo.embed, { x: PAGE_W / 2 - gw / 2, y: PAGE_H / 2 - gh / 2, width: gw, height: gh, opacity: 0.06 });
    const sw = 130; const sh = sw * (logo.height / logo.width);
    ...
    page.drawText("European Private Banking · Est. 1897", { x: MARGIN, y: PAGE_H - 54, size: 9, font, color: TEXT });
  }
  ...
  const dateLine = `Issued: ${fmtDateTime(ctx.generatedAt)}`;
...
function drawAmountHero(ctx: Ctx) {
  ...
  const badgeSize = 10.5; const padX = 16; const padY = 6;
  const bw = bold.widthOfTextAtSize(badge, badgeSize);
  const boxX = PAGE_W / 2 - (bw + padX * 2) / 2;
  page.drawRectangle({ x: boxX, y: ctx.cursorY - (badgeSize + padY * 2), width: bw + padX * 2, height: badgeSize + padY * 2, color: statusColor(data), opacity: 0.94 });
  page.drawText(badge, { x: boxX + padX, y: ctx.cursorY - badgeSize - padY + 2, size: badgeSize, font: bold, color: rgb(1, 1, 1) });
  ctx.cursorY -= badgeSize + padY * 2 + 16;
}

function drawPartyPanels(ctx: Ctx) {
  const { page, font, bold, data } = ctx;
  const gap = 12; const panelW = (PAGE_W - MARGIN * 2 - gap) / 2;
  const innerPad = 10; const titleH = 20; const rowH = 21;
  const partyRows = (p: ReceiptParty) => {
    const rows: Array<{ label: string; value: string; lines: 1 | 2 }> = [{ label: "Name", value: p.name || "—", lines: 1 }];
  ...
  const weight = (rows: typeof senderRows) => rows.reduce((acc, r) => acc + r.lines, 0);
  const rowUnits = Math.max(weight(senderRows), weight(benefRows));
  const panelH = titleH + innerPad + rowUnits * rowH + 2;
  const top = ctx.cursorY; const y = top - panelH;

  const drawPanel = (x: number, title: string, rows: typeof senderRows) => {
    page.drawRectangle({ x, y, width: panelW, height: panelH, color: PANEL_BG, borderColor: SOFT_BORDER, borderWidth: 0.5 });
    ...
    for (const row of rows) {
      ...
      if (row.lines === 2) {
        ...
        if (lines.length > 1) page.drawText(fitText(lines.slice(1).join(" "), font, 9, maxW), { x: x + innerPad, y: rowY - 22, size: 9, font, color: NAVY });
      } else {
...
function drawDataPanel(ctx: Ctx, title: string, rows: Array<[string, string]>) {
  ...
  const innerPad = 12; const panelX = MARGIN; const panelW = PAGE_W - MARGIN * 2;
  const rowH = 15.5; const titleH = 20;
  const panelH = titleH + innerPad + rows.length * rowH + 4;
  const top = ctx.cursorY; const y = top - panelH;
  page.drawRectangle({ x: panelX, y, width: panelW, height: panelH, color: PANEL_BG, borderColor: SOFT_BORDER, borderWidth: 0.5 });
  ...
  for (const [label, value] of rows) {
    ...
    page.drawText(fitText(value || "—", font, 9.5, panelW - innerPad * 2 - labelW), { x: panelX + innerPad + labelW, y: rowY, size: 9.5, font, color: NAVY });
    rowY -= rowH;
  }
  ctx.cursorY = y - 14;
}
...
function drawSettlementAndSecurity(ctx: Ctx, qrImg: PDFImage | null) {
  const { page, font, bold, data } = ctx;
  const gap = 12; const totalW = PAGE_W - MARGIN * 2;
  const sumW = Math.round(totalW * 0.52); const secW = totalW - sumW - gap;
  const panelH = 96; const top = ctx.cursorY; const y = top - panelH;

  const sumX = MARGIN;
  page.drawRectangle({ x: sumX, y, width: sumW, height: panelH, color: PANEL_BG, borderColor: SOFT_BORDER, borderWidth: 0.5 });
  ...
  const rows: Array<[string, string, boolean]> = [["Transfer amount", fmtMoney(data.amount, data.currency), false]];
  if (data.fee > 0) rows.push(["Transfer fee", fmtMoney(data.fee, data.currency), false]);
  rows.push([data.direction === "credit" ? "Total credited" : "Total debited", `${data.direction === "credit" ? "+" : "-"}${fmtMoney(data.totalSettled, data.currency)}`, true]);

  let rowY = top - 20 - 16;
  for (const [label, value, emphasis] of rows) {
    if (emphasis) page.drawLine({ start: { x: sumX + 12, y: rowY + 12 }, end: { x: sumX + sumW - 12, y: rowY + 12 }, thickness: 0.8, color: GOLD });
    const lFont = emphasis ? bold : font; const size = emphasis ? 11 : 9.5;
    ...
    page.drawText(value, { x: sumX + sumW - 12 - vW, y: rowY, size, font: lFont, color: emphasis ? (data.direction === "credit" ? GREEN : NAVY_DEEP) : NAVY });
    rowY -= 19;
  }

  const secX = sumX + sumW + gap;
  page.drawRectangle({ x: secX, y, width: secW, height: panelH, color: rgb(0.957, 0.965, 0.976), borderColor: SOFT_BORDER, borderWidth: 0.5 });
  const qrSize = 68;
  if (qrImg) page.drawImage(qrImg, { x: secX + 10, y: y + (panelH - qrSize) / 2, width: qrSize, height: qrSize });
  const textX = secX + 10 + qrSize + 8; const textW = secW - (textX - secX) - 8;
  let ty = top - 16;
  page.drawText("DOCUMENT AUTHENTICATION", { x: textX, y: ty, size: 7, font: bold, color: GOLD_DARK });
  ty -= 10;
  page.drawText("Verify this receipt:", { x: textX, y: ty, size: 6.5, font, color: TEXT });
  ty -= 9;
  page.drawText("aldwycheuropeancapital.com/verify", { x: textX, y: ty, size: 6.5, font, color: NAVY });
  ty -= 14;
  page.drawText("INTEGRITY DIGEST (SHA-256)", { x: textX, y: ty, size: 6, font: bold, color: MUTED });
  ty -= 8;
  const digest = (ctx.integrityHash.match(/.{1,8}/g) || []).join(" ").toUpperCase();
  const dLines = wrapText(digest, font, 5.8, textW).slice(0, 3);
  for (const ln of dLines) { page.drawText(ln, { x: textX, y: ty, size: 5.8, font, color: MUTED }); ty -= 7.5; }
  ctx.cursorY = y - 12;
}
...
async function embedQR(pdf: PDFDocument, url: string) {
  ...
  return await pdf.embedPng(Uint8Array.from(Buffer.from(base64, "base64")));
}

function drawFooter(ctx: Ctx, pageNum: number, pageCount: number) {
  const { page, font, bold } = ctx;
  const top = FOOTER_RESERVED;
  page.drawLine({ start: { x: MARGIN, y: top + 2 }, end: { x: PAGE_W - MARGIN, y: top + 2 }, thickness: 0.5, color: SOFT_BORDER });
  const lines = [
  ...
  let fy = top - 14;
  for (const ln of lines) { page.drawText(ln, { x: MARGIN, y: fy, size: 7.5, font, color: MUTED }); fy -= 10; }
  page.drawText(`Receipt ${ctx.data.receiptNumber} · Transaction ${ctx.data.reference}`, { x: MARGIN, y: 18, size: 8, font: bold, color: TEXT });
...
export async function generateReceiptPDF(data: ReceiptData): Promise<Uint8Array> {
  ...
  const integrityHash = generateIntegrityHash(data);
  const verificationUrl = `https://aldwycheuropeancapital.com/verify/${data.receiptNumber}`;
  ...
  if (logoBytes) {
    try { const img = await pdf.embedPng(logoBytes); logo = { width: img.width, height: img.height, embed: img }; }
    catch { try { const img = await pdf.embedJpg(logoBytes); logo = { width: img.width, height: img.height, embed: img }; } catch { logo = null; } }
  }

  const page = pdf.addPage([PAGE_W, PAGE_H]);
  const ctx: Ctx = { pdf, page, font, bold, logo, data, cursorY: PAGE_H - 104, integrityHash, generatedAt };
  ...
  const details: Array<[string, string]> = [
    ["Receipt number", data.receiptNumber], ["Transaction reference", data.reference],
    ["Transfer type", data.transferKind], ["Payment rail", data.rail],
    ["Channel", data.channel], ["Initiated", fmtDateTime(data.initiatedAt)],
    [data.status === "completed" ? "Settled" : "Expected settlement", data.completedAt ? fmtDateTime(data.completedAt) : "Pending clearing confirmation"],
  ];
  ...
  const qrImg = await embedQR(pdf, verificationUrl).catch(() => null);
  drawSettlementAndSecurity(ctx, qrImg);

  const pages = pdf.getPages();
  pages.forEach((p, i) => drawFooter({ ...ctx, page: p }, i + 1, pages.length));
